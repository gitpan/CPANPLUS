...
CPANPLUS::Tools::Module provides simple ways to query and possibly load any of
...
to $CPANPLUS::Tools::Module::VERBOSE
...
sub check_install {
    ...
    DIR: for my $dir ( @INC ) {
        ...
        if ( ref $dir ) {
            ...
            if (!UNIVERSAL::isa($fh, 'GLOB')) {
                warn loc("Can't open %1: %2", $file, $!) if $args->{verbose};
                next;
            }
    ...
    if( !defined $href->{version} ) {
        ...
        {   local $^W;  # silly version warnings
...
This function uses the C<load> function from CPANPLUS::Tools::Load under the
hood.
...
load. A synonym is C<complain>. The default is to use the value of
...
sub can_load {
    ...
    BLOCK: {
        ...
        for my $mod ( keys %$href ) {
            ...
            {   local $^W;  # silly version warnings
                if (    !$args->{nocache}
                    ...
                    $error = loc("Already attempted to use '%1', which was unsuccessful", $mod),
                ...
                if( !$mod_data or !defined $mod_data->{file} ) {
                    $error = loc(qq[Could not find or check module '%1'],$mod);
                    $CACHE->{$mod}->{usable} = 0;
                    last BLOCK;
                }
        ...
        for my $mod ( @load ) {
            ...
            ### $CACHE and return 0
                ...
                $error = loc(qq[Module '%1' is not uptodate!],$mod);
                $CACHE->{$mod}->{usable} = 0;
                last BLOCK;
            }
        }
    ...
    if( defined $error ) {
        $ERROR = $error;
        Carp::carp( "$error " . loc('[THIS MAY BE A PROBLEM!]') ) if $args->{verbose};
        return undef;
    } else {
        return 1;
    }
...
sub requires {
    my $who = shift or return undef;

    unless( check_install( module => $who ) ) {
        warn loc( qq[You do not have module '%1' installed], $who ) if $VERBOSE;
        return undef;
    }
...
The behaviour of CPANPLUS::Tools::Module can be altered by changing the
following global variables:

=head2 $CPANPLUS::Tools::Module::VERBOSE

This controls whether CPANPLUS::Tools::Module will issue warnings and
explenations as to why certain things may have failed. If you set it
to 0, CPANPLUS::Tools::Module will not output any warnings.
The default is 1;

=head2 $CPANPLUS::Tools::Module::CACHE
...
=head2 $CPANPLUS::Tools::Module::ERROR
