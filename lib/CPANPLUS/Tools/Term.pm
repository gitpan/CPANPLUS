...
sub get_reply {
    ...
    my $args = check( $tmpl, \%hash, $VERBOSE )
                or ( warn( loc(qq[Could not parse arguments]) ), return );
...
sub _tt_readline {
    ...
    if ($AUTOREPLY) {
        warn loc(q[You have $AUTOREPLY set to true, but did not provide a default!])
...
sub parse_options {
    ...
    while ( $input =~ s/(?:\A|[\W\s]+)--?([-\w]+=("|').+?\2)//   or
            $input =~ s/(?:\A|[\W\s]+)--?([-\w]+=\S+)//          or
            $input =~ s/(?:\A|[\W\s]+)--?([-\w]+)//
    ) {
        my $match = $1;
        ...
        if( $match =~ /^([-\w]+)=("|')(.+?)\2$/ ) {
            ...
            warn loc(qq[I do not understand option "%1"\n], $match) if $VERBOSE;
        }
    }
    $input =~ s/^\s+//; $input =~ s/\s+$//;
...
CPANPLUS::Tools::Term is a transparent way of eliminating the overhead of having
...
the user can pick from and a default and CPANPLUS::Tools::Term will DWYM.

For asking a yes or no question, there's even a shortcut.

=head1 How it works

CPANPLUS::Tools::Term places itself at the back of the Term::ReadLine @ISA
...
of the types that the CPANPLUS::Tools::Check C<check> function allows, so please
...
The behaviour of CPANPLUS::Tools::Term can be altered by changing the following
global variables:

=head2 $CPANPLUS::Tools::Term::VERBOSE

This controls whether CPANPLUS::Tools::Term will issue warnings and explenations
as to why certain things may have failed. If you set it to 0,
CPANPLUS::Tools::Term will not output any warnings.
The default is 1;

=head2 $CPANPLUS::Tools::Term::AUTOREPLY
...
=head2 $CPANPLUS::Tools::Term::INVALID
...
C<CPANPLUS::Tools::Check>, C<Term::ReadLine>
