# $File: //member/autrijus/cpanplus/dist/lib/CPANPLUS/TesterGuide.pod $ $Author: autrijus $
# $Revision: #2 $ $Change: 4020 $ $DateTime: 2002/04/29 05:02:50 $
...
at L<http://egb.elixus.org/cpanplus-dist.tar.gz>; it should contain
...
questions, since we will be using B<LWP>, B<Mail::Send> and B<IPC::Run>
...
below to your B<Mail::Audit> filter [4]:
 ...
 fork || exec("sleep 3600 && cpansmoke -aps $1 >/dev/null 2>&1")
...
In case you're wondering, the C<sleep 3600> line is due to the fact
that a PAUSE upload needs several minutes to appear on I<www.cpan.org>.
...
(but do consider upgrading to B<Mail::Audit>):
 ...
 :0hc
 * ^Subject: CPAN Upload:
 |sh -c "sleep 3600 && grep Subject|cut -f4 -d' '|xargs cpansmoke -aps >/dev/null 2>&1"
...
Unfortunately, the process of editing and sending out reports
remains as the only non-programmable part, since the F<cpantest>
script doesn't exist as a module.  Although Skud has a
B<CPAN::Test::Reporter> on CPAN, its format is sadly out-of-sync
with F<cpantest>, and may generate invalid reports (as of version
C<0.02>).  Any offers to back-port the F<cpantest> script into
that module will be greatly appreciated.
...
With the advent of bug-tracking service [6] and automatic testing,
...
projects like B<Module::Build> (C<make>-less installs), B<NAPC>
